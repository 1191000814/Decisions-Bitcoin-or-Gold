...
DELAY_DAYS = 5 # 推迟买的天数, 如果为负就是提前买了
RANDOM = True # 是否开启随机模型
...
for day in range(Trading.DAYS - 1):
    ...
    # 比特币买卖策略
    flag = 0
    if (trading.day + DELAY_DAYS < Trading.DAYS) & (trading.day + DELAY_DAYS > 0):
        '''买卖天数故意偏移'''
        if RANDOM is False:
            flag = DELAY_DAYS
        else: # 随机决定是否推迟
            flag = DELAY_DAYS * randint(-1, 1)
        trading.day += flag
...
np.savetxt("../data/资产值变化-随机提前或推迟5天.csv", capital_list)
