...
class Macd:
    ...
    def get_tran_days(form = True):
        ...
        for day in range(1, days): # 无法预测未来一天是否能买
            if (macd_list[day - 1] < 0) & (macd_list[day] > 0):  # 由负变正->买
                tran_days[day] = 1
            elif (macd_list[day - 1] > 0) & (macd_list[day] < 0):  # 由负正变负->卖
                tran_days[day] = -1

        '''
        buy_day = -3
        for day in range(days):
            if tran_days[day] == 1: # 找到一个买点
                buy_day = day

            elif (tran_days[day] == -1) & (day - buy_day <= 1): # 卖点与上一个买点的间隔不超过三天
                tran_days[buy_day] = 0
                tran_days[day] = 0
